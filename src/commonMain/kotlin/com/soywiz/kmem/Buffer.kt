...
class Buffer private constructor(val mem: MemBuffer, val size: Int = mem.size) {
	...
	companion object {
		...
		fun alloc(size: Int): Buffer = Buffer(MemBufferAlloc(size.sizeAligned()), size)
		fun wrap(buffer: MemBuffer, size: Int = buffer.size): Buffer = Buffer(buffer, size)
		fun wrap(array: ByteArray): Buffer = Buffer(MemBufferWrap(array), array.size)

		operator fun invoke(size: Int, direct: Boolean) = Buffer(if (direct) MemBufferAlloc(size.sizeAligned()) else MemBufferAllocNoDirect(size.sizeAligned()), size)
		operator fun invoke(size: Int): Buffer = Buffer(MemBufferAlloc(size.sizeAligned()), size)
		operator fun invoke(buffer: MemBuffer, size: Int = buffer.size): Buffer = Buffer(buffer, size)
		operator fun invoke(array: ByteArray): Buffer = Buffer(MemBufferWrap(array), array.size)

		fun copy(src: Buffer, srcPos: Int, dst: Buffer, dstPos: Int, length: Int): Unit =
			arraycopy(src.buffer, srcPos, dst.buffer, dstPos, length)

		fun copy(src: Buffer, srcPos: Int, dst: ByteArray, dstPos: Int, length: Int): Unit =
			arraycopy(src.buffer, srcPos, dst, dstPos, length)

		fun copy(src: ByteArray, srcPos: Int, dst: Buffer, dstPos: Int, length: Int): Unit =
			arraycopy(src, srcPos, dst.buffer, dstPos, length)

		fun copyAligned(src: Buffer, srcPosAligned: Int, dst: ShortArray, dstPosAligned: Int, length: Int): Unit =
			arraycopy(src.buffer, srcPosAligned, dst, dstPosAligned, length)

		fun copyAligned(src: ShortArray, srcPosAligned: Int, dst: Buffer, dstPosAligned: Int, length: Int): Unit =
			arraycopy(src, srcPosAligned, dst.buffer, dstPosAligned, length)

		fun copyAligned(src: Buffer, srcPosAligned: Int, dst: IntArray, dstPosAligned: Int, length: Int): Unit =
			arraycopy(src.buffer, srcPosAligned, dst, dstPosAligned, length)

		fun copyAligned(src: IntArray, srcPosAligned: Int, dst: Buffer, dstPosAligned: Int, length: Int): Unit =
			arraycopy(src, srcPosAligned, dst.buffer, dstPosAligned, length)

		fun copyAligned(src: Buffer, srcPosAligned: Int, dst: FloatArray, dstPosAligned: Int, length: Int): Unit =
			arraycopy(src.buffer, srcPosAligned, dst, dstPosAligned, length)

		fun copyAligned(src: FloatArray, srcPosAligned: Int, dst: Buffer, dstPosAligned: Int, length: Int): Unit =
			arraycopy(src, srcPosAligned, dst.buffer, dstPosAligned, length)
	}
...
inline fun <T> buffer(size: Int, callback: (Buffer) -> T): T = Buffer(size).run(callback)

fun Buffer.setFloats(offset: Int, data: FloatArray, dataOffset: Int, count: Int) = this.apply {
